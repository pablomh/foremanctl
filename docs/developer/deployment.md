...
The integrated `foreman-proxy` deployment follows a similar split between identity and reachability. `foreman_proxy_name` and `foreman_proxy_url` remain the public smart-proxy identity that Foreman stores, while `foreman_proxy_registration_url` is only the internal reachability target used by the colocated Foreman container, currently `https://foreman-proxy:8443`. For the default certificate flow, foremanctl reissues the host certificate when that internal alias becomes newly required, including the common two-step deployment flow where `foreman-proxy` is enabled after the initial `foreman` deploy. For `certificate_source: custom_server`, foremanctl cannot add SANs to the user-provided server certificate, so integrated deployments require that certificate to already include `foreman-proxy`; otherwise the deploy fails before proxy registration. Proxy-only deployments continue to use the certificate-bundle hostname for both public identity and the reachability check.
