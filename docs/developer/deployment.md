...
The integrated `foreman-proxy` deployment follows a similar split between identity and reachability. `foreman_proxy_name` remains the public smart-proxy identity and external entry point, while `foreman_proxy_registration_url` is the operational URL that Foreman stores and uses from inside the integrated deployment, currently `https://foreman-proxy:8443`. For the default certificate flow, foremanctl reissues the host certificate when that internal alias becomes newly required, including the common two-step deployment flow where `foreman-proxy` is enabled after the initial `foreman` deploy. For `certificate_source: custom_server`, foremanctl cannot add SANs to the user-provided server certificate, so integrated deployments require that certificate to already include `foreman-proxy`; otherwise the deploy fails before proxy registration. Proxy-only deployments continue to use the certificate-bundle hostname for both public identity and registration.
