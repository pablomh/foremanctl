...
The integrated `foreman-proxy` deployment follows a similar split between identity and reachability. `foreman_proxy_fqdn` remains the public smart-proxy identity and entry point, while Foreman uses a bridge-local operational URL by default so colocated containers can reach the proxy reliably after the network refactor. If you need Foreman to store a different operational URL, such as a CI-only shortcut alias or a load-balancer DNS name, set `--registration-url` explicitly. For the default certificate flow, foremanctl automatically reissues the host certificate when that effective registration hostname becomes newly required, including the common two-step deployment flow where `foreman-proxy` is enabled after the initial `foreman` deploy. For `certificate_source: custom_server`, foremanctl cannot add SANs to the user-provided server certificate, so any explicit registration URL override, plus the integrated `foreman-proxy` alias, must already be covered by that certificate. Proxy-only deployments continue to use the auth-bundle hostname as both the public identity and the default registration URL, with the same explicit `--registration-url` override available when needed.
