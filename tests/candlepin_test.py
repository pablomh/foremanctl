...
def test_candlepin_service(user_service):
    assert user_service("candlepin").is_running


def test_candlepin_status(server):
    status = foremanctl_run(server, "podman exec foreman curl --cacert /etc/foreman/katello-default-ca.crt --silent --output /dev/null --write-out '%{http_code}' https://candlepin:23443/candlepin/status")
    assert status.succeeded
    assert status.stdout == '200'


def test_artemis_auth(server):
    cmd = foremanctl_run(server, 'podman exec foreman bash -c \'echo "" | openssl s_client -CAfile /etc/foreman/katello-default-ca.crt -cert /etc/foreman/client_cert.pem -key /etc/foreman/client_key.pem -connect candlepin:61613 -servername candlepin\'')
...
def test_tls(server):
    ...
    result = foremanctl_run(server, f'podman exec foreman bash -c "echo Q | openssl s_client -connect candlepin:23443 -tls1_2 -CAfile {ca} 2>&1"')
    ...
    for flag in ['-tls1_3', '-tls1_1', '-tls1']:
        result = foremanctl_run(server, f'podman exec foreman bash -c "echo Q | openssl s_client -connect candlepin:23443 {flag} -CAfile {ca} 2>&1"')
