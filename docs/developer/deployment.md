...
The integrated `foreman-proxy` deployment now uses a single registration URL model. `foreman_proxy_fqdn` remains the public smart-proxy identity and entry point, and Foreman stores that same real FQDN URL by default. If you need Foreman to store a different endpoint, such as a CI-only shortcut alias or a load-balancer DNS name, set `--registration-url` explicitly. For the default certificate flow, foremanctl automatically reissues the host certificate when an explicit registration override introduces a new hostname, including the common two-step deployment flow where `foreman-proxy` is enabled after the initial `foreman` deploy. For `certificate_source: custom_server`, foremanctl cannot add SANs to the user-provided server certificate, so any explicit registration URL override must already be covered by that certificate. Proxy-only deployments continue to use the auth-bundle hostname as both the public identity and the default registration URL, with the same explicit `--registration-url` override available when needed.
