...
The `foreman-proxy` deployment always registers the smart proxy with Foreman under its own real FQDN URL (`foreman_proxy_name`/`foreman_proxy_url`); this identity is unaffected by the bridge-network split. `--registration-url` is a separate, optional, purely informational override: when set, it is written into the proxy's own `settings.d/registration.yml` as the endpoint the proxy tells newly-registering hosts to use (for example, a load-balancer DNS name in front of multiple proxies), with no effect on how Foreman manages the proxy and no certificate SAN requirement of its own.
