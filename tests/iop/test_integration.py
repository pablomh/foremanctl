...
def test_iop_ingress_endpoint(server, user):
    result = run_as(server, user, f"podman exec iop-core-ingress curl -s -o /dev/null -w '%{{http_code}}' http://localhost:8080/ 2>/dev/null || echo '000'")
    assert result.rc == 0
...
def test_iop_puptoo_metrics_endpoint(server, user):
    result = run_as(server, user, f"podman exec iop-core-puptoo curl -s -o /dev/null -w '%{{http_code}}' http://localhost:8000/metrics 2>/dev/null || echo '000'")
    assert result.rc == 0
...
def test_iop_yuptoo_endpoint(server, user):
    result = run_as(server, user, f"podman exec iop-core-yuptoo curl -s -o /dev/null -w '%{{http_code}}' http://localhost:5005/ 2>/dev/null || echo '000'")
    assert result.rc == 0
...
def test_iop_gateway_endpoint(server, user):
    result = run_as(server, user, f"podman exec iop-core-gateway curl -s -o /dev/null -w '%{{http_code}}' http://localhost:24443/ 2>/dev/null || echo '000'")
    assert result.rc == 0


def test_iop_gateway_api_ingress_endpoint(server, user):
    result = run_as(server, user, f"podman exec iop-core-gateway curl -s -o /dev/null -w '%{{http_code}}' http://localhost:24443/api/ingress 2>/dev/null || echo '000'")
    assert result.rc == 0
...
def test_iop_inventory_mq_endpoint(server, user):
    result = run_as(server, user, f"podman run --rm --network=iop-core-network quay.io/iop/host-inventory:foreman-3.18 curl http://iop-core-host-inventory:9126/ 2>/dev/null || echo '000'")
    assert result.rc == 0


def test_iop_inventory_api_health_endpoint(server, user):
    result = run_as(server, user, f"podman run --rm --network=iop-core-network quay.io/iop/host-inventory:foreman-3.18 curl -s -o /dev/null -w '%{{http_code}}' http://iop-core-host-inventory-api:8081/health 2>/dev/null || echo '000'")
    assert "200" in result.stdout
...
def test_iop_advisor_api_endpoint(server, user):
    result = run_as(server, user, f"podman run --rm --network=iop-core-network quay.io/iop/advisor-backend:foreman-3.18 curl -f http://iop-service-advisor-backend-api:8000/ 2>/dev/null || echo '000'")
    assert result.rc == 0
...
def test_iop_remediations_api_endpoint(server, user):
    result = run_as(server, user, f"podman exec iop-service-remediations-api curl -s -o /dev/null -w '%{{http_code}}' http://localhost:9002/ 2>/dev/null || echo '000'")
    assert result.rc == 0
