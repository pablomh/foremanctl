...
def test_podman_network_backend(server, user):
    ...
    if user:
        cmd = run_as(server, user, f"podman info --format '{{{{.Host.NetworkBackend}}}}'")
    else:
...
def test_podman_network_create_user(server, user):
    ...
    remove_cmd = run_as(server, user, f"podman network rm {network_name}")
...
def test_podman_network_dns_resolution(server, user):
    ...
    try:
        # Create network
        run_as(server, user, f"podman network create {network_name}")

        # Start first container
        run_as(server, user, f"podman run -d --name {container1} --network {network_name} quay.io/centos/centos:stream9 sleep 300")

        # Start second container and test DNS resolution to first container using getent
        dns_cmd = run_as(server, user, f"podman run --rm --network {network_name} quay.io/centos/centos:stream9 getent hosts {container1}")
        ...
        # Cleanup
        run_as(server, user, f"podman rm -f {container1} {container2} || true")
        run_as(server, user, f"podman network rm {network_name} || true")
