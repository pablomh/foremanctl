...
def test_advisor_backend_service(server, user):
...
def test_advisor_api_quadlet_file(server, user):
    user_home = get_user_home(server, user)
    quadlet_file = server.file(f"{user_home}/.config/containers/systemd/iop-service-advisor-backend-api.container")
    assert quadlet_file.exists
    assert quadlet_file.is_file


def test_advisor_service_quadlet_file(server, user):
    user_home = get_user_home(server, user)
    quadlet_file = server.file(f"{user_home}/.config/containers/systemd/iop-service-advisor-backend-service.container")
    assert quadlet_file.exists
    assert quadlet_file.is_file


def test_advisor_api_service_dependencies(server, user):
    result = server.run(f"systemctl --machine={user}@ --user show iop-service-advisor-backend-api --property=After")
    assert result.succeeded
    assert "iop-core-kafka.service" in result.stdout


def test_advisor_service_dependencies(server, user):
    result = server.run(f"systemctl --machine={user}@ --user show iop-service-advisor-backend-service --property=After")
    assert result.succeeded
    assert "iop-core-kafka.service" in result.stdout
